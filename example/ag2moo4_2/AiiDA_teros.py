...
class AiiDATEROSWorkChain(WorkChain):
    ...
    def result_ternary(self):
        try:
            ...
            logP_P0 = 1
            kB = 1.38064852e-23 * ureg.J / ureg.K  # Boltzmann's constant
            mu_ex = 0.4 * ureg.eV
            ...
            for n, calc in enumerate(self.ctx.relax_calcs,start=1):
                ...
                self.report(f"Processing slab calculation for {label}")
                self.process_slab_calculation(calc, label)
                
                self.report(f"Calculating gamma and storing results for {label}")
                self.calculate_gamma_and_store_results(label)

                self.report(f"Appending termination data for {label}")
    ...
    def process_bulk_calculation(self):
        try:
            ...
            self.ctx.energy_bulk = outcar_ase_bulk.get_potential_energy() * ureg.eV / divide_minimal_comp
    ...
    def process_slab_calculation(self, calc, label):
        try:
            ...
            energy_slab = outcar_ase.get_potential_energy() * ureg.eV
            cell_params = outcar_ase.cell.cellpar() * ureg.angstrom
    ...
    def calculate_gamma_and_store_results(self, label):
        try:
            ...
            self.report(f"Elements count: {elements_count}")
            self.report(f"Composition bulk: {composition_bulk}")
            self.report(f"Energy bulk: {energy_bulk}")

            delta_me_ag = composition_bulk[1] * elements_count[self.ctx.unique_symbols[0]] - composition_bulk[0] * elements_count[self.ctx.unique_symbols[1]]
            delta_me_o = composition_bulk[1] * elements_count[self.ctx.unique_symbols[2]] - composition_bulk[2] * elements_count[self.ctx.unique_symbols[1]]
            ...
            energy_o2 = self.inputs.total_energy_o2.value * ureg.eV
            self.report(f"====================")
            self.report(f"Calculating psi for {label}")
            self.report(f"energy_slab: {self.ctx.dict_results[label]['energy_slab']}")
            self.report(f"elements_count: {elements_count}")
            self.report(f"energy_bulk: {energy_bulk}")
            self.report(f"composition_bulk: {composition_bulk}")
            self.report(f"energy_ag: {energy_ag}")
            self.report(f"delta_me_ag: {delta_me_ag}")
            self.report(f"energy_o2: {energy_o2}")
            self.report(f"delta_me_o: {delta_me_o}")
            self.report(f"====================")
            ...
            delta_mu = np.array([0, lim_delta_o]) * ureg.eV  # Only need two points
            ...
            # Calculate gamma for the first and last values of delta_mu
            gamma = []
            for m in delta_mu:
                try:
                    gamma_value = factor * (
                        psi
                        - self.ctx.dict_results[label]['Delta_Me_Ag'] * 0 * ureg.eV  # delta_mu_ag is zero
                        - self.ctx.dict_results[label]['Delta_Me_O'] * m
                    )
                    self.report(f"====================")
                    self.report(f"Calculated gamma value: {gamma_value}")
                    self.report(f"Factor: {factor}")
                    self.report(f"Psi: {psi}")
                    self.report(f"Delta_Me_Ag: {self.ctx.dict_results[label]['Delta_Me_Ag']}")
                    self.report(f"Delta_Me_O: {self.ctx.dict_results[label]['Delta_Me_O']}")
                    self.report(f"Delta_mu: {m}")
                    self.report(f"====================")
    ...
    def append_termination_data(self, termination_data, label):
        try:
            ...
            energy_slab = self.ctx.dict_results[label]['energy_slab']
            a = self.ctx.dict_results[label]['a']
            b = self.ctx.dict_results[label]['b']
            
            self.report(f"delta_me_o: {delta_me_o}")
            self.report(f"delta_me_ag: {delta_me_ag}")
            self.report(f"energy_slab: {energy_slab}")
            self.report(f"a: {a}")
            self.report(f"b: {b}")

            # Append data to termination_data list
            termination_data.append({
                'label': label,
                'delta_me_o': delta_me_o,
                'delta_me_ag': delta_me_ag,
                'energy_slab': energy_slab.magnitude,
                'a': a.magnitude,
                'b': b.magnitude
            })
            self.report(f"Appended termination data for {label}")
        except Exception as e:
            self.report(f"Error in append_termination_data for label {label}: {e}")
    ...
    def precompute_phase_diagram_parameters(self):
        try:
            ...
            self.report(f"Composition Bulk: {composition_bulk}")
            self.report(f"Limit Delta Ag: {lim_delta_ag}")
            self.report(f"Limit Delta O: {lim_delta_o}")
            self.report(f"Delta Ag: {delta_ag}")
            self.report(f"Delta O: {delta_o}")
    ...
    def plot_gammas_ternary(self):
        ...
        plt.tight_layout()
        plt.savefig(f"{self.inputs.path_to_graphs.value}/thermo_results/ternary/surface_free_energies.pdf", bbox_inches="tight")
    ...
    def plot_phase_diagram(self):
        ...
        delta_ag = self.ctx.dict_results[self.ctx.exemple_label]['delta_ag']
        delta_o = self.ctx.dict_results[self.ctx.exemple_label]['delta_o']
